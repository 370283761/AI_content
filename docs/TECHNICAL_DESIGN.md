...
| POST | `/api/scenes/:id/split` | 人工拆分场景（M1） |
| POST | `/api/scenes/:id/merge-next` | 与下一场景合并（M1） |
...
| POST | `/api/shots/:id/split` | 人工拆分镜头（M1） |
| POST | `/api/shots/:id/merge-next` | 与下一镜头合并（M1） |
| GET/POST | `/api/projects/:id/locations` | 地点资产列表与创建 |
| PATCH/DELETE | `/api/locations/:id` | 地点资产更新与软删除 |
| GET/POST | `/api/projects/:id/props` | 道具资产列表与创建 |
| PATCH/DELETE | `/api/props/:id` | 道具资产更新与软删除 |
